    ...
    public class AgendaRelatorio : EntityBase, IPersisteableEntity
    {
        ...
        public static String ControladoriaTaxaQUERY(DateTime dtFrom, DateTime dtTo, String[] oper, String[] estp)
        {
            String qry = String.Concat(
                ...
                "   case ",
                "       WHEN  operadora_id IN (4) THEN 'SP' ",
                "       WHEN  operadora_id IN (5) THEN 'RJ' ",
                ...
                "   case ",
                "       WHEN contrato_vigencia < '2013-07-01 00:00:00' then 'UBRASP' ",
                "       WHEN operadora_id = 16 then 'UBRASP' ",
                "       ELSE 'UBRASP' ",
                "   end as Cedente ",
        ...
        public static string CobrancasQUERY(DateTime dtFrom, DateTime dtTo, String[] oper, String[] estp)
        {
            string query = string.Concat(
                "select beneficiario_nome,beneficiario_matriculaAssociativa,beneficiario_matriculaFuncional,cobranca_valor,cobranca_datavencimento,case cobranca_pago when 1 then 'Sim' else 'Nao' end as pago,cobranca_datapagto ",
                "   from beneficiario ",
                ...
                "       inner join cobranca on cobranca_propostaId = contrato_id ",
                "   where ",
                "       cobranca_cancelada=0 and (cobranca_pago=1 or (cobranca_pago=0 and contrato_inativo=0 and contrato_cancelado=0) )",
                "		and cobranca_dataVencimento between '", dtFrom.ToString("yyyy-MM-dd 00:00:00.000"), "' and '", dtTo.ToString("yyyy-MM-dd 23:59:59.998"), "' ",
                "       and contrato_operadoraId IN (", String.Join(",", oper), ")",
                "       and contrato_estipulanteId IN (", String.Join(",", estp), ")",
                "   order by cobranca_datavencimento, beneficiario_nome");

            return query;
        }

        public static string CobrancasNaoGeradasQUERY(DateTime dtFrom, DateTime dtTo, String[] oper, String[] estp)
        {
            string query = string.Concat(
                "select contrato_id,contrato_numero,beneficiario_nome,operadora_nome,contratoadm_descricao,plano_descricao ",
                "   from contrato ",
                ...
                "   where ",
                "       contrato_vencimento <= '", dtFrom.ToString("yyyy-MM-dd"), "' and ",
                "       contrato_cancelado=0 and contrato_inativo=0 and contrato_rascunho=0 and ",
                "		contrato_id not in ( select cobranca_propostaId from cobranca where cobranca_cancelada=0 and cobranca_dataVencimento between '", dtFrom.ToString("yyyy-MM-dd 00:00:00.000"), "' and '", dtTo.ToString("yyyy-MM-dd 23:59:59.998"), "') ",
                "       and contrato_operadoraId IN (", String.Join(",", oper), ")",
                "       and contrato_estipulanteId IN (", String.Join(",", estp), ")",
                "   order by operadora_nome,contratoadm_descricao,beneficiario_nome");
        ...
        public static String ContasReceberPagoQUERY(DateTime dtFrom, DateTime dtTo, String[] oper, String[] estp)
        {
            String qry = String.Concat(
                    ...
                    "		inner join operadora on operadora_id = contrato_operadoraId ",
                    "		inner join estipulante on estipulante_id = contrato_estipulanteId ",
                    "       left join usuario on usuario_id=contrato_donoId ",
                    "       inner join contratoadm on contratoadm_id=contrato_contratoadmid ",
                    "       inner join cobranca on cobranca_propostaId=contrato_id ",
                    "       inner join plano on  contrato.contrato_planoId = plano_Id and contrato.contrato_contratoAdmId = plano.plano_contratoId ",
        ...
        {
            String qry = String.Concat(
                    "select operadora_nome,estipulante_descricao,estipulante_carteira,contrato_numero,contratobeneficiario_numeroSequencia,beneficiario_nome,beneficiario_cpf,datediff(dd,beneficiario_dataNascimento,cobranca.cobranca_dataVencimento) / 365 as IDADE,CONVERT(varchar(14), beneficiario.beneficiario_dataNascimento, 103) as beneficiario_dataNascimento,CONVERT(varchar(14), contrato_vigencia, 103) as contrato_vigencia,CONVERT(varchar(14), contrato_datacancelamento, 103) as contrato_datacancelamento,CONVERT(varchar(14), cobranca_dataVencimento, 103) as cobranca_dataVencimento,CONVERT(varchar(14), cobranca_dataPagto, 103) as cobranca_dataPagto,cobranca_valor,cobranca_valorPagto,contrato_codcobranca,cobranca_parcela,plano_descricao,CONVERT(varchar(14), cobranca_dataCriacao, 103) as cobranca_dataCriacao,usuario_nome,",
                    "   case ",
                    "       WHEN  operadora_id IN (4) THEN 'SP' ",
                    "       WHEN  operadora_id IN (5) THEN 'RJ' ",
                //"       WHEN  operadora_id IN (3,4,5,6,8,9,16,22) THEN 'SP' ",
