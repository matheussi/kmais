    ...
    public partial class contrato : PageBase
    {
        ...
        void CarregaContrato()
        {
            ...
            if (contrato.VencimentoAlterado > 0)
                txtVencimentoAlterado.Text = contrato.VencimentoAlterado.ToString();
        ...
        void CarregaFichaDeSaude(Boolean atualizar)
        {
            ...
            if (ViewState["fic_" + cboBeneficiarioFicha.SelectedValue] == null || atualizar)
            {
                ...
                //else if(!IsPostBack)
                //{
                //    var def = ItemDeclaracaoSaude.Carregar(cboOperadora.SelectedValue);
                //    if (def != null)
                //    {
                //        int i = 0;
                //        foreach (ListItem benef in cboBeneficiarioFicha.Items)
                //        {
                //            List<ItemDeclaracaoSaudeINSTANCIA> instancias = new List<ItemDeclaracaoSaudeINSTANCIA>();

                //            foreach (var item in def)
                //            {
                //                ItemDeclaracaoSaudeINSTANCIA instancia = new ItemDeclaracaoSaudeINSTANCIA();
                //                instancia.BeneficiarioID = benef.Value;
                //                instancia.Data = DateTime.Now;
                //                instancia.Descricao = item.Texto;
                //                instancia.ItemDeclaracaoCodigo = item.Codigo;
                //                instancia.ItemDeclaracaoID = item.ID;
                //                instancia.Sim = false;
                //                instancia.Salvar();
                ...
                //            if (i == 0) { dlFicha.DataSource = lista; }
                //            ViewState["fic_" + cboBeneficiarioFicha.SelectedValue] = lista;
                //            i++;
                //        }
                //    }
        ...
        void CarregaAdicionais(Boolean atualizar)
        {
            ...
            if (lista != null)
            {
                ...
                {
                    if (ad.AdicionalTipo == 0 || ad.AdicionalTipo == 3) taxas.Add(ad);
        ...
        {
            Adicional a = new Adicional(cboAdicionaisParaAdicionar.SelectedValue);
            a.Carregar();
            ...
            if(a.Tipo != 3)
                ab.FormaPagto = 11; // desc folha
            else
                ab.FormaPagto = 31; //boleto
        ...
        protected void gridAdicional_OnRowDataBound(Object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                AdicionalBeneficiario ab = e.Row.DataItem as AdicionalBeneficiario;
                if (ab.AdicionalTipo == 3)
                {
                    ((TextBox)e.Row.Cells[1].Controls[1]).Text = "0,00";
                    ((TextBox)e.Row.Cells[1].Controls[1]).Enabled = false;
                    ((TextBox)e.Row.Cells[1].Controls[1]).ToolTip = "O valor considerado será o cadastrado no adicional";

                    ((DropDownList)e.Row.Cells[2].Controls[1]).Enabled = false;
                    ((DropDownList)e.Row.Cells[2].Controls[1]).SelectedIndex = 1;
                }
            }
        }
        ...
        void ExibeSumario()
        {
            ...
            if (taxas != null)
            {
                ...
                foreach (AdicionalBeneficiario ab in taxas)
                {
                    ...
                    else if (ab.AdicionalTipo == 3) //normal
                    {
                        Adicional produto = new Adicional();
                        produto.ID = ab.AdicionalID;
                        produto.Carregar();

                        if (contrato.ID == null)
                            valor += Adicional.CalculaValor(ab.AdicionalID, ab.BeneficiarioID, idade);
                        else
                            valor += Adicional.CalculaValor(ab.AdicionalID, ab.BeneficiarioID, idade, contrato.Vigencia, null);
                    }
                }
        ...
        protected void cmdSalvar_Click(Object sender, EventArgs e)
        {
            ...
            if (Operadora.IsSalutar(cboOperadora.SelectedValue) && txtCarenciaCodigo.Text.Trim() != "")
            {
                if (!Operadora.ValidaPRCSalutar(txtCarenciaCodigo.Text))
                {
                    String validos = String.Join(", ", Operadora.SalutarPRCs);
            ...
            if (txtVencimentoAlterado.Text.Trim() != "")
            {
                int dia = base.CToInt(txtVencimentoAlterado.Text);

                if (dia < 1 || dia > 28)
                {
                    base.Alerta(MPE, ref litAlert, "Atenção:<br>O dia do novo vencimento deve estar entre 1 e 28.", upnlAlerta);
                    return;
                }
            }
            ...
            if(TitularID == null)//if (cboBeneficiarioFicha.Items.Count == 0)
            {
        ...
        void Salvar(Object usuarioLiberadorId, Boolean rascunho)
        {
            ...
            contrato.VencimentoAlterado = base.CToInt(txtVencimentoAlterado.Text);
            ...
            if (ViewState[IDKey] != null)
            {
                ...
                ContratoFacade.Instance.Salvar(contrato, titular, this.Dependentes, fichas, usuarioLiberadorId, adicionaisContratados, conferencia, this.ValorTotalProposta);
            ...
            if (novo) { Session[FilialIDKey] = cboFilial.SelectedValue; Response.Redirect("contrato.aspx"); }
            else 
            {
                String msg = "";
                ContratoFacade.Instance.AtualizaValorDeCobrancas(contrato.ID, out msg);
        ...
        void calculaValorCobranca_Boleto() //chamado por ConfiguraAtendimento
        {
            ...
            if (cobrancas != null && cobrancas.Count > 0)
            {
                ...
                txtValorCob.Text = Contrato.CalculaValorDaProposta2(this.contratoId, proxVencto, null, false, true, ref composite, true).ToString("N2");
            }
            else
            {
        ...
        protected void cmdDemonsPagtoQuali_Click(Object sender, EventArgs e)
        {
            ...
            String ano = (DateTime.Now.Year - 1).ToString(); //ConfigurationManager.AppSettings["anoRefDemonstrPagtos"];
        ...
        protected void imgDemonstPagtoQualiMail_Click(Object sender, EventArgs e)
        {
            ...
            if (Convert.ToString(contrato.ID) == "43623") //neste caso deve inverter os dados de dependente e titular
            {
                ...
                cTitular.BeneficiarioNome = dep.Nome;
                cTitular.BeneficiarioCPF = dep.CPF;
            }
            ...
            if (cobrancas == null || cobrancas.Count == 0)
            {
                base.Alerta(null, this, "_semIR", "Não há cobranças para o ano exercício " + ano);
                pm.CloseSingleCommandInstance(); pm.Dispose();
                return;
            }
            ...
            try
            {
                ...
                string ret = proxy.EnviaEmail(
                    ...
                    txtEmailAtendimento.Text, //"denis@wedigi.com.br",//
            ...
            catch { base.Alerta(null, this, "_ok", "E-mail não pôde ser enviado."); }
        ...
        protected void gridCobranca_RowCommand(Object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "email" || e.CommandName == "print")
            {
                ...
                //if(cobranca.Carteira == 101) finalUrl = "/boleto/santander2.aspx" + uri;
                ...
                try
                {
                    ...
                    html = sr.ReadToEnd().Replace(@"D:\http\ubrasp\web\var\boleto_file\", @"http://ubrasp.iphotel.info/images/boleto/"); //BoletoNetBarra.gif // Replace("Versão demonstrativa do Gerador de Boletos (www.boletoasp.com.br)", "");
                    sr.Close();
                    response.Close();
                ...
                if (e.CommandName == "email" && !string.IsNullOrEmpty(email))
                {
                    MailMessage msg = new MailMessage(
                        new MailAddress(ConfigurationManager.AppSettings["mailFrom"], ConfigurationManager.AppSettings["mailFromName"]),
                        new MailAddress("temp@temp.com.br"));
                    ...
                    try
                    {
                        if (email.IndexOf(';') > -1)
                        {
                            ...
                            for (int i = 0; i < arr.Length; i++)
                            {
                                ...
                                if (i == 0) msg.To.Add(arr[i]);
                                else        msg.Bcc.Add(arr[i]);
                            }
                        }
                        else
                        {
                    ...
                    {
                        base.Alerta(null, this, "_err", "Erro ao adicionar email.");
                        return;
                    }

                    msg.Subject = "[UBRASP] Atendimento - Boleto";
                    msg.IsBodyHtml = true;

                    //msg.Bcc.Add("denis.goncalves@wedigi.com.br");

                    //msg.Body = html.Replace("src='/images/boleto/", "src='http://ubrasp.iphotel.info/images/boleto/");
                    ...
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(ret)) throw new ApplicationException(ret);
                    }
                    catch 
                    {
                        throw;
        ...
        protected void cmdGerarCobranca_Click(Object sender, EventArgs e)
        {
            ...
            ////Para não gerar muitos meses a frente
